...
def ner_quest_document_results(request, task_pk, doc_pk):
    ...
    if len(opponent_pks):
        ...
        opponent_dict = {
            ...
            'level': Level.objects.filter(user_id=opponent_pks[0], requirement__task_type='ner').first().get_name()
        }
...
def ner_quest(request, quest_pk):
    '''View that serves required html and starts the NER JS library
