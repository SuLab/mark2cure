/**
 * Notes from Runjie Guan, Feb 10, 2017:
 * When upgrading marionette, I set "start" region to be the only region in Tree, so to get "start" region, simply use Tree.getRegion()
 */

var initialize_tree = function() {
    ...
    Tree.getRegion().onShow = function(m, c) {
...
RelationTaskCollection = Backbone.Collection.extend({
    ...
    next: function() {
        ...
        if (next_relationship) {
            var self = this;
            if (Tree['convoChannel']) {
                Tree['convoChannel'].unbind();
            }
            ...
            if (current_idx >= 1) {
                ...
                if (previous_concepts['c1'].id != concepts['c1'].id) {
                    concepts['c1']['fadeIn'] = true;
                }
                ;
                if (previous_concepts['c2'].id != concepts['c2'].id) {
                    concepts['c2']['fadeIn'] = true;
                }
                ;
            }
            ...
            var view_options = {
                collection: coll,
                concepts: current_relationship.get('concepts'),
                choice: false,
                first_draw: true
            };

            Tree.getRegion().show(new RelationCompositeView(view_options));
